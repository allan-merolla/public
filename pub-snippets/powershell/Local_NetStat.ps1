PowerShell – Netstat Counter Connections

$hostnames = "hostname1,hostname2"
$csvpath = "d:\temp\sftp_netstat_log.csv"
...
while ($true)
{
    for ($i=0; $i -lt $hostnamesArr.Length; $i++)
    {
        $hostname=$hostnamesArr[$i]
        write-host "Host: " $hostname " Check: " $i
        $table = @()
        ...
        foreach ($entry in $netstat)
        {
            ...
            foreach ($row in $table)
            {
                ...
                {
                    write-host $row "was found: " $found
                    $found=$true
