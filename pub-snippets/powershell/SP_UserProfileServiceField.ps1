...
if($ProfileManager){
    foreach ($Site in $(Get-SPSite -Limit All | ? {!$_.Url.Contains("Office_Viewing_Service_Cache")})){
        ...
        foreach ($User in $($RootWeb.SiteUsers)){
            if ($User.UserLogin -match "<user_to_fix>")
            {
                ...
                if ($ProfileManager.UserExists($($User.UserLogin))){
                      ...
                      $UserItem = $UserList.GetItems($Query)[0]gg;
                      ForEach ($Map in $PropertyMap){
                            ...
                            if($PropName -eq "JobTitle")
                            {
                               $UserItem[$PropName] = "Coordinator Transport Policy & Partnerships"
                            }
                            if($PropName -eq "SPS-JobTitle")
                            {
                               $UserItem[$PropName] = "Coordinator Transport Policy & Partnerships"
                            }
                            if($PropName -eq "Job Title")
                            {
                               $UserItem[$PropName] = "Coordinator Transport Policy & Partnerships"
                            }
                       }
